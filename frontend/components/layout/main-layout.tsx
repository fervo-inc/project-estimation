      ...
      <SidebarProvider>
        <div className="flex h-screen bg-background">
              ...
              <nav className="flex flex-1 flex-col gap-1 p-2">
                {navigation.map((item) => (
                  <Button
                    ...
                    className={cn('w-full justify-start gap-2', item.current && 'bg-secondary')}
                    asChild
                  >
                    <Link href={item.href}>
          ...
          <div className="flex flex-1 flex-col overflow-hidden">
            ...
            <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 mx-auto w-full max-w-7xl">{children}</main>
          </div>
        </div>
      </SidebarProvider>
    </ThemeProvider>
